...
g. 对ZPLAY Ads广告源进行配置，在Class Name处输入适配类名称，即将下图中ZPLAYAds替换为适配器类名称，如示例中激励视频为```ZPLAYAdsAdMobAdapter```，插屏广告为```ZPLAYAdsAdMobInterstitialAdapter```，Parameter第一个值为ZPLAY Ads平台申请的应用ID，第二个值为ZPLAY Ads平台申请的广告位ID，注意这两个值的顺序不能更改，且之间只有一个空格，点击“DONE”完成ZPLAY Ads的配置
